...
router.get('/', async (req, res, next) => {
  try {
    const {
      ...
      fecha_desde, fecha_hasta, estado_pago, q, rubro_id, subrubro_gasto_id,
      limit = 100, offset = 0,
    } = req.query;
    ...
    if (subrubro_gasto_id) { conditions.push(`e.subrubro_gasto_id = $${idx++}`); params.push(subrubro_gasto_id); }
    if (sucursal_id)  { conditions.push(`e.sucursal_id = $${idx++}`);  params.push(sucursal_id); }
...
router.post('/:id/pago', async (req, res, next) => {
  ...
  try {
    ...
    // Si se pagó con "Efectivo Caja Fuerte", descontar de la caja fuerte
    // del medio (fallback: la sucursal del egreso) y asentarlo en el ledger.
    await registrarEgresosCajaFuerteDeMedios(client, [{ medio_pago_id, monto: montoPago }], {

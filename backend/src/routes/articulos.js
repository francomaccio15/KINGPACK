...
// ?desc_cats=id:N  descuentos adicionales por categoría (override de descuento global)
router.get('/pdf-precios', async (req, res, next) => {
  try {
    ...
    res.setHeader('Content-Disposition', `inline; filename="lista-precios-${listaNombre.toLowerCase().replace(/\s+/g, '-')}.pdf"`);
    doc.pipe(res);
    ...
    for (const art of rows) {
      ...
      doc.fillColor('#f0f0f0').fontSize(8).font('Helvetica').text(art.nombre, 135, rowY, { width: PAGE_W - 155 });
    ...
    const [{ rows }, { rows: countRows }] = await Promise.all([
      pool.query(`
        SELECT
          ...
          a.costo_base, a.costo_flete, a.margen_aplicado,
        ...
        LEFT JOIN lista_precio_items lpi ON lpi.articulo_id = a.id
        LEFT JOIN listas_precios     lp  ON lp.id = lpi.lista_id AND lp.activo = true
        WHERE ${where}
        GROUP BY a.id, a.codigo, a.nombre, a.costo_base, a.costo_flete,
                 a.margen_aplicado, a.precio_madre, a.activo,
                 c.id, c.nombre, c.margen_default, iv.porcentaje,
                 st.cantidad_total, st.stock_bajo
        ORDER BY c.nombre NULLS LAST, a.nombre

...
//   pendiente   → en_transito : debita stock de origen
//   pendiente   → cancelado   : sin cambio de stock
//   en_transito → recibido    : acredita stock en destino
//   en_transito → cancelado   : devuelve stock al origen (reversión)
    ...
    if (nuevoEstado === 'en_transito') {
      ...
      for (const item of items) {
        ...
        await client.query(`
          ...
          VALUES ($1, $2, $3, $4)
        `, [item.articulo_id, t.sucursal_origen_id, -item.cantidad, `Traspaso enviado → ${t.sucursal_destino_nombre} (${id})`]);
      }
      ...
      for (const item of items) {
        ...
        await client.query(`
          INSERT INTO ajustes_stock (articulo_id, sucursal_id, cantidad_delta, motivo)
          VALUES ($1, $2, $3, $4)
        `, [item.articulo_id, t.sucursal_destino_id, item.cantidad, `Traspaso recibido ← ${t.sucursal_origen_nombre} (${id})`]);
      }
      ...
      // Si estaba en_transito, devolver stock al origen
      if (t.estado === 'en_transito') {
        for (const item of items) {
          ...
          await client.query(`
            INSERT INTO ajustes_stock (articulo_id, sucursal_id, cantidad_delta, motivo)
            VALUES ($1, $2, $3, $4)
          `, [item.articulo_id, t.sucursal_origen_id, item.cantidad, `Traspaso cancelado — reversión (${id})`]);
        }
      }

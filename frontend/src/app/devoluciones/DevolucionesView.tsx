...
interface Props {
  devolucionesIniciales: Devolucion[];
  totalCount: number;
  clientes: Cliente[];
  sucursales: Sucursal[];
}
...
const FORMA_LABEL: Record<FormaDevolucion, string> = {
  ...
  cambio:           'Cambio mercadería',
};

function FormaBadge({ forma }: { forma: FormaDevolucion }) {
  const colors: Record<FormaDevolucion, string> = {
    efectivo:         'bg-green-500/15 border-green-500/30 text-green-300',
    cuenta_corriente: 'bg-sky-500/15 border-sky-500/30 text-sky-300',
    transferencia:    'bg-violet-500/15 border-violet-500/30 text-violet-300',
    cambio:           'bg-amber-500/15 border-amber-500/30 text-amber-300',
  };
  return (
    <span className={`inline-flex items-center text-2xs md:text-[10px] font-bold px-2 py-0.5 rounded-full border ${colors[forma]}`}>
      {FORMA_LABEL[forma]}
    </span>
  );
}
...
export default function DevolucionesView({ devolucionesIniciales, totalCount, clientes, sucursales }: Props) {
  const { user } = useAuth();
  const isAdmin  = user?.rol === 'administrador'; // anular y eliminar solo admin

  const [devs,         setDevs]         = useState<Devolucion[]>(devolucionesIniciales);
  ...
  const filtered = devs.filter(d => {
    ...
    if (q) {
      ...
      return (
        d.cliente_razon_social?.toLowerCase().includes(lower) ||
        d.motivo.toLowerCase().includes(lower) ||
        String(d.numero ?? '').includes(lower) ||
        (d.numero_referencia ?? '').toLowerCase().includes(lower)
      );
    }
    return true;
  });

  const handleCreate = (d: Devolucion) => setDevs(prev => [d, ...prev]);

  const handleAnular = async (id: string) => {
    setAnulando(id);
    try {
      const r = await apiFetch(`/api/devoluciones/${id}/anular`, { method: 'PATCH' });
      if (r.ok) setDevs(prev => prev.map(d => d.id === id ? { ...d, estado: 'anulada' } : d));
    } finally {
      setAnulando(null);
      setConfirmId(null);
    }
  ...
  const handleEliminar = async (id: string) => {
    setEliminando(id);
    try {
      const r = await apiFetch(`/api/devoluciones/${id}`, { method: 'DELETE' });
      if (r.ok) setDevs(prev => prev.filter(d => d.id !== id));
    } finally {
      setEliminando(null);
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h2 className="text-lg md:text-2xl font-bold uppercase tracking-wide">Devolución de Mercadería</h2>
          </div>
          <p className="text-sm text-kp-gray pl-3">
            {filtered.length} {filtered.length === 1 ? 'devolución' : 'devoluciones'}
            {filtroEstado ? ` · ${filtroEstado}` : ''} · ventas sin factura
          </p>
        </div>
        <button
        ...
        <table data-rt="1" className="min-w-full text-sm">
          <thead>
            <tr className="bg-kp-surface2 border-b border-kp-border">
              ...
              <th className="text-left px-4 py-3 text-kp-gray uppercase tracking-widest text-xs font-semibold">Forma</th>
          ...
          <tbody className="bg-kp-surface divide-y divide-kp-border">
            {filtered.length === 0 ? (
              ...
              filtered.map(d => (
                <tr key={d.id} className="hover:bg-kp-surface2 transition-colors group">
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className="text-xs text-kp-gray-lt font-mono">
                      {d.numero ? `N° ${String(d.numero).padStart(6, '0')}` : '—'}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3 max-w-[180px]">
                    <p className="text-xs font-medium text-kp-white truncate">
                      {d.cliente_razon_social ?? 'Consumidor final'}
                    </p>
                    {d.cliente_cuit && (
                      <p className="text-2xs md:text-[10px] text-kp-gray font-mono">{d.cliente_cuit}</p>
                    )}
                  </td>
                  <td className="px-4 py-3 max-w-[200px]">
                    <p className="text-xs text-kp-gray-lt truncate">{d.motivo}</p>
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <FormaBadge forma={d.forma_devolucion} />
                  </td>
                  <td className="px-4 py-3 text-right whitespace-nowrap">
                    <span className={`text-sm font-bold tabular-nums ${d.estado === 'anulada' ? 'text-kp-gray line-through' : 'text-kp-red'}`}>
                      {ars.format(d.total)}
                    </span>
                  </td>
                      ...
                      {isAdmin && (
                        <>
                          {d.estado === 'emitida' && (
                            confirmId === d.id ? (
                              <div className="flex items-center gap-1">
                                <button onClick={() => handleAnular(d.id)} disabled={anulando === d.id}
                          ...
                          {confirmDelId === d.id ? (
                            ...
                            <button onClick={() => setConfirmDelId(d.id)} title="Eliminar"

...
function ArticuloInput({
  ...
  const handleChange = (v: string) => { setQuery(v); onChange(v); };
...
const MOTIVOS_PRESET = [
  ...
  'Producto fallado / defectuoso',
  'Producto equivocado',
  'Cambio por otro artículo',
  'Cliente arrepentido',
];

const FORMAS: { value: FormaDevolucion; label: string }[] = [
  { value: 'efectivo',         label: 'Efectivo (sale de caja)' },
  { value: 'cuenta_corriente', label: 'Cuenta corriente (saldo a favor)' },
...
interface Props {
  clientes: Cliente[];
  sucursales: Sucursal[];
  onCreate: (d: Devolucion) => void;
  onClose: () => void;
}

const emptyItem = (): DevItem => ({ descripcion: '', cantidad: 1, precio_unitario: 0, subtotal: 0, articulo_id: undefined });

export default function NuevaDevolucion({ clientes, sucursales, onCreate, onClose }: Props) {
  ...
  const [formaDevolucion, setFormaDevolucion]  = useState<FormaDevolucion>('efectivo');
  const [items,           setItems]            = useState<DevItem[]>([emptyItem()]);
  const [saving,          setSaving]           = useState(false);
  const [error,           setError]            = useState('');

  // — Cargar venta completa por número —
  ...
  const buscarVenta = async () => {
    const query = ventaNumero.trim();
    if (!query) return;
    setBuscandoVenta(true);
    setErrorVenta('');
    setVentaCargada(null);
    try {
      const r = await apiFetch(`/api/ventas?q=${encodeURIComponent(query)}&limit=5`);
      const data = await r.json();
      const match = (data.ventas ?? []).find((v: Record<string, unknown>) => String(v.numero) === query) ?? data.ventas?.[0];
  ...
  // Totales — la devolución no lleva IVA: total = subtotal
  const subtotal = items.reduce((s, it) => s + it.cantidad * it.precio_unitario, 0);
  const total    = Math.round(subtotal * 100) / 100;

  const updateItem = (i: number, field: keyof DevItem, val: string | number) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formaDevolucion === 'cuenta_corriente' && !clienteId) {
      setError('Seleccioná un cliente para devolver a cuenta corriente'); return;
    }
    if (formaDevolucion === 'efectivo' && !sucursalId) {
      setError('Seleccioná la sucursal para devolver en efectivo'); return;
    }

    setSaving(true);
    try {
      const r = await apiFetch('/api/devoluciones', {
        method: 'POST',
        body: JSON.stringify({
  ...
  const ayudaForma = (() => {
    switch (formaDevolucion) {
      case 'efectivo':         return 'Se registrará un egreso por el total en la caja abierta de la sucursal.';
      case 'cuenta_corriente': return clienteId ? 'Se acreditará el total al saldo a favor del cliente.' : 'Seleccioná un cliente para acreditar el saldo.';
      case 'transferencia':    return 'Devolución por transferencia. No afecta la caja ni la cuenta corriente.';
      case 'cambio':           return 'Solo reingresa la mercadería al stock. No se devuelve dinero.';
    }
  })();
      ...
      <div className="w-full max-w-3xl mx-4 bg-kp-surface rounded-2xl border border-kp-border shadow-2xl shadow-black/60">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-kp-border bg-kp-surface2 rounded-t-2xl">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-base font-bold uppercase tracking-wide">Nueva Devolución de Mercadería</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className={`rounded-xl border transition-colors ${modoVenta ? 'border-blue-500/30 bg-blue-500/5' : 'border-kp-border bg-kp-surface2'}`}>
            <button
              type="button"
              onClick={() => { setModoVenta(v => !v); setVentaNumero(''); setVentaCargada(null); setErrorVenta(''); }}
              className="w-full flex items-center justify-between px-4 py-3 text-left"
            >
              <div className="flex items-center gap-2">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className={`w-4 h-4 ${modoVenta ? 'text-blue-400' : 'text-kp-gray'}`}>
                  <path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/>
                </svg>
                <span className={`text-xs font-bold uppercase tracking-wider ${modoVenta ? 'text-blue-400' : 'text-kp-gray'}`}>
                  Cargar desde una venta
                </span>
                {ventaCargada && (
                  <span className="ml-1 text-xs font-semibold text-green-400">— Venta #{ventaCargada.numero} cargada ✓</span>
                )}
              </div>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className={`w-4 h-4 text-kp-gray transition-transform ${modoVenta ? 'rotate-180' : ''}`}>
                ...
                {ventaCargada && (
                  <div className="flex items-start gap-2 text-xs bg-green-500/10 border border-green-500/30 rounded-lg px-3 py-2.5 text-green-400">
                    ...
                    <div>
                      ...
                      <p className="text-green-400/70 mt-0.5">Podés ajustar los ítems, cantidades y motivo antes de registrar.</p>
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className={labelCls}>Venta / Doc. de referencia</label>
              <input
                type="text" value={numRef}
                onChange={e => setNumRef(e.target.value)}
                placeholder="Ej: Venta #437 / Ticket"
                className={inputCls}
              />
            </div>
          </div>
          ...
          {/* Motivo */}
          <div>
            <label className={labelCls}>Motivo de la devolución *</label>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className={labelCls + ' mb-0'}>Mercadería devuelta *</label>
              <button
          ...
          {/* Forma de devolución + Totales */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-start">
            <div>
              <label className={labelCls}>Forma de devolución</label>
              <select
                value={formaDevolucion}
                onChange={e => setFormaDevolucion(e.target.value as FormaDevolucion)}
                className={selectCls}
              >
                {FORMAS.map(f => (
            ...
            <div className="bg-kp-surface2 border border-kp-border rounded-xl p-4 space-y-2">
              ...
              <div className="flex justify-between font-bold text-base border-t border-kp-border pt-2">
                <span className="text-kp-white">TOTAL a devolver</span>

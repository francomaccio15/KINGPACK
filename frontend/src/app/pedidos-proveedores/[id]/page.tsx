...
export default async function DetallePedidoPage({ params }: { params: { id: string } }) {
  ...
  const eventos = recepciones.map((ev: any) => {
    ...
    const tipo: 'correccion' | 'parcial' | 'completa' =
      esCorreccion ? 'correccion' : (completo ? 'completa' : 'parcial');
    return { fecha: ev.fecha, usuario_nombre: ev.usuario_nombre, detalle: ev.detalle, tipo };
  });
  ...
  return (
    <section className="space-y-6 max-w-4xl">
      ...
      <HistorialRecepciones eventos={eventos} />

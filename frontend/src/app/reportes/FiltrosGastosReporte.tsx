...
export default function FiltrosGastosReporte({ fechaDesde, fechaHasta, rubroId, rubros }: Props) {
  ...
  return (
    <form onSubmit={aplicar} className="flex flex-wrap items-end gap-3">
      ...
      <div className="flex flex-col gap-1">
        <label className="text-2xs md:text-[10px] font-semibold uppercase tracking-widest text-kp-gray">Rubro</label>
        <select value={rubro} onChange={e => setRubro(e.target.value)}
          className="bg-kp-surface2 border border-kp-border rounded-lg px-3 py-2 min-h-touch md:min-h-touch-sm text-base md:text-sm text-kp-white focus:outline-none focus:border-kp-red min-w-[180px]">

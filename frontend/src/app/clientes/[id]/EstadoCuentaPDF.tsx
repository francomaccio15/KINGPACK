...
export default function EstadoCuentaPDF({ clienteId }: { clienteId: string }) {
  ...
  const abrir = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/clientes/${clienteId}/pdf-estado-cuenta`, {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
      if (!res.ok) throw new Error('Error al generar estado de cuenta');
  ...
  return (
    ...
    >
      ...
      {loading ? 'Generando...' : 'Imprimir Cuenta'}
    </button>
  );
}

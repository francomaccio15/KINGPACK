...
// Datos de un egreso existente para el modo edición.
export interface EgresoEdicion {
  id: string;
  ...
  sucursal_id: string;
  subrubro_gasto_id: string | null;
  descripcion: string;
  neto_gravado: string | number;
  neto_no_gravado: string | number;
  iva_21: string | number;
  iva_105: string | number;
  percepciones_ib: string | number;
  otros_impuestos: string | number;
  total: string | number;
  fecha_vencimiento_pago: string | null;
  bonificaciones: { pct: number; monto: number }[] | null;
  items: {
    ...
    sucursal_imputacion_id: string | null;
  }[];
}

const toStr = (v: string | number | null | undefined) => {
  const n = parseFloat(String(v ?? ''));
  return !v || isNaN(n) || n === 0 ? '' : String(n);
};
...
export default function EgresoForm({ edicion }: { edicion?: EgresoEdicion | null }) {
  ...
  const [tipoOp, setTipoOp] = useState<TipoOperacion>(edicion?.tipo_operacion ?? 'compra_mercaderia');
  const [tipoComp, setTipoComp] = useState<TipoComprobante>(edicion?.tipo_comprobante ?? 'factura_a');
  const [puntoVenta, setPuntoVenta] = useState(edicion?.punto_venta ?? '');
  const [nroComprobante, setNroComprobante] = useState(edicion?.numero_comprobante ?? '');
  const [fechaEmision, setFechaEmision] = useState(
    edicion?.fecha_emision ? edicion.fecha_emision.split('T')[0] : new Date().toISOString().split('T')[0]
  );
  const [proveedorId, setProveedorId] = useState(edicion?.proveedor_id ?? '');
  const [sucursalId, setSucursalId] = useState(edicion?.sucursal_id ?? '');
  const [subrubroId, setSubrubroId] = useState(edicion?.subrubro_gasto_id ?? '');
  const [descripcion, setDescripcion] = useState(edicion?.descripcion ?? '');
  const [items, setItems] = useState<LineItem[]>(
    edicion?.items?.map((it, idx) => ({
      key: idx,
      articulo_id: it.articulo_id ?? '',
      descripcion: it.descripcion ?? '',
      cantidad: parseFloat(String(it.cantidad)) || 0,
      precio_unitario: parseFloat(String(it.precio_unitario)) || 0,
      descuento_pct: parseFloat(String(it.descuento_pct)) || 0,
  ...
  // Montos fiscales. Para comprobante informal quedan vacíos (igual que en el
  // alta): el total lo maneja el subtotal de ítems, no el desglose fiscal.
  const edInformal = !edicion?.tipo_comprobante || edicion?.tipo_comprobante === 'informal';
  const fiscalInit = (v: string | number | null | undefined) => (edInformal ? '' : toStr(v));
  const [netoGravado, setNetoGravado] = useState(fiscalInit(edicion?.neto_gravado));
  const [netoNoGravado, setNetoNoGravado] = useState(fiscalInit(edicion?.neto_no_gravado));
  const [iva21, setIva21] = useState(fiscalInit(edicion?.iva_21));
  ...
  // En edición NO se resetea (el tipo queda bloqueado y los datos vienen cargados).
  const skipResetTipo = useRef(esEdicion);
  useEffect(() => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await apiFetch(
        esEdicion ? `/api/egresos/${edicion!.id}` : `/api/egresos`,
        {
      ...
      if (esEdicion) {
        router.push(`/gastos/${edicion!.id}`);
        router.refresh();
      } else if (data.pedido_creado?.id) {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-12">
      ...
      <div className={sectionCls}>
        <h3 className="text-xs font-bold uppercase tracking-widest text-kp-gray">
          {esEdicion ? 'Vencimiento' : 'Forma de pago'}
        </h3>

        <div className="grid grid-cols-2 gap-4">
          {/* El pago no se edita desde acá: sólo egresos sin pagos son editables */}
          {!esEdicion && (
            <div>
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex gap-3 flex-shrink-0">
          <Link href={esEdicion ? `/gastos/${edicion!.id}` : '/gastos'}
          ...
          >
            {saving ? <><Spinner /> Guardando…</> : esEdicion ? 'Guardar Cambios' : 'Confirmar Egreso'}
          </button>
        </div>
      </div>

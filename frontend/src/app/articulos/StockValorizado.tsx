    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="rounded-xl bg-kp-surface border border-kp-border p-4">
          <p className="text-2xs md:text-[11px] uppercase tracking-widest text-kp-gray font-semibold">Artículos</p>
          <p className="text-2xl font-bold mt-1">{filas.length}</p>
        </div>
        <div className="rounded-xl bg-kp-surface border border-kp-border p-4">
          <p className="text-2xs md:text-[11px] uppercase tracking-widest text-kp-gray font-semibold">Unidades en depósito</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {nombresSuc.map(n => (
          <div key={n} className="rounded-xl bg-kp-surface border border-kp-red/30 p-4">
            <p className="text-2xs md:text-[11px] uppercase tracking-widest text-kp-red/90 font-semibold">Valor stock · {n} (a costo)</p>
            <p className="text-2xl font-bold mt-1 text-kp-red">{ars.format(totalValorSuc[n] ?? 0)}</p>
            <p className="text-2xs md:text-[11px] text-kp-gray mt-1">{num.format(totalUnidSuc[n] ?? 0)} unidades</p>
          </div>
        ))}
        <div className="rounded-xl bg-kp-red/10 border border-kp-red/50 p-4">
          <p className="text-2xs md:text-[11px] uppercase tracking-widest text-kp-red font-semibold">Valor total del stock (a costo)</p>
          <p className="text-2xl font-bold mt-1 text-kp-red">{ars.format(totalValor)}</p>
          <p className="text-2xs md:text-[11px] text-kp-gray mt-1">{num.format(totalUnid)} unidades</p>
        </div>
      </div>
            ...
            {filas.length === 0 ? (
              ...
              <tr key={a.id} className="border-b border-kp-border/60 hover:bg-kp-surface2/50">
                ...
                <td className="px-3 py-2">
                  ...
                  {a.categoria && (
                    <span className="block text-2xs md:text-[11px] text-kp-gray">{a.categoria}</span>
                  )}
                </td>
            ...
            <tr className="bg-kp-surface2 border-t-2 border-kp-red/40">
              ...
              {nombresSuc.map(n => (
                <td key={n} className="px-3 py-3 text-center whitespace-nowrap">
                  ...
                  <span className="block text-2xs md:text-[11px] font-bold text-kp-red">{ars.format(totalValorSuc[n] ?? 0)}</span>
                </td>
              ))}

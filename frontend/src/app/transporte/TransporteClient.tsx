  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <p className="mt-1 text-2xs md:text-[11px] text-kp-gray/70">Positivo = ya le debías al transporte antes de empezar.</p>
      </div>
              ...
              {movs.map(m => (
                <tr key={m.id} className="bg-kp-surface hover:bg-kp-surface2 transition-colors">
                  ...
                  <td className="px-3 py-2 text-center">
                    <span className={`inline-block px-2 py-0.5 text-2xs md:text-[10px] font-semibold rounded border ${m.tipo === 'pago' ? 'bg-green-500/10 text-green-400 border-green-500/30' : 'bg-amber-500/10 text-amber-400 border-amber-500/30'}`}>
    ...
    <section className="space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg md:text-2xl font-bold uppercase tracking-wide">Transporte</h2>
      ...
      {/* Modales */}
      {modalCrear && (
        <Modal open title="Nuevo transporte" onClose={() => setModalCrear(false)}>
          <FormTransporte onGuardar={() => { setModalCrear(false); cargar(); }} onCerrar={() => setModalCrear(false)} />
        </Modal>
      )}
      {modalEditar && (
        <Modal open title="Editar transporte" onClose={() => setModalEditar(null)}>
          <FormTransporte inicial={modalEditar} onGuardar={() => { setModalEditar(null); cargar(); }} onCerrar={() => setModalEditar(null)} />
        </Modal>
      )}
      {modalMov && (
        <Modal open title={`Movimientos — ${modalMov.razon_social}`} onClose={() => setModalMov(null)} size="xl">
